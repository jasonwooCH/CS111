...
interrupt(registers_t *reg)
{
	...
	case INT_SYS_EXIT:
		...
		if (current->wait_queue != NULL)
		{
			current->wait_queue->p_state = P_RUNNABLE;
			current->wait_queue->p_registers.reg_eax = current->p_exit_status;
		}

		current->p_state = P_EMPTY; // when inside if, all red
		schedule();
	...
	case INT_SYS_WAIT: {
		...
		{
			proc_array[p].wait_queue = current;
...
copy_stack(process_t *dest, process_t *src)
{
	...
	uint32_t gap = src_stack_top - src_stack_bottom;

	dest_stack_top = PROC1_STACK_ADDR + dest_pid* PROC_STACK_SIZE;
	dest_stack_bottom = dest_stack_top - gap;

	memcpy((void*)dest_stack_bottom, (const void*)src_stack_bottom, gap);
